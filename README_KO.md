...
<sub>방법: 프로토콜별로 같은 수를 뽑은 표본, 프록시를 통한 엔드투엔드 요청, 8초 제한. 대표 수치는 그 표본의 비율이며, `stats/reliability.json`의 `listSuccessRate`가 실제 공개된 프로토콜 구성으로 다시 가중합니다. — `http://api.ipify.org/, http://icanhazip.com/, https://api.ipify.org/, https://icanhazip.com/`</sub>
...
- **[LLM API pricing list](https://github.com/xyzs996/llm-api-pricing/blob/main/README_KO.md)** — 한 행에 적힌 가격은 요청 한 번의 가격이 아닙니다. 다른 가격표가 빠뜨리는 열이 여기에는 있습니다. 프롬프트가 길어지면 켜지는 두 번째의 더 비싼 요금표——게다가 비싼 요율이 적용되는 것은 기준을 넘은 토큰만이 아니라 그 요청의 모든 토큰입니다. 글에서 인용한 모든 수치도 [한 표](https://xyzs996.github.io/llm-api-pricing/figures.html)에 있고, 각 행마다 출처 문장이 함께 있습니다 ([JSON](https://cdn.jsdelivr.net/gh/xyzs996/llm-api-pricing@main/data/figures.json) / [CSV](https://cdn.jsdelivr.net/gh/xyzs996/llm-api-pricing@main/data/figures.csv)). 먼저 [토큰 요금이 실제로 어디로 가는지](https://xyzs996.github.io/llm-api-pricing/topics/token-optimization.html)부터.
