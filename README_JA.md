...
<sub>方法：プロトコルごとに同数を抽出し、プロキシ経由でエンドツーエンド取得、8 秒タイムアウト。見出しの数値はその標本の比率で、`stats/reliability.json` の `listSuccessRate` が公開されたプロトコル構成で重み付けし直します。 — `http://api.ipify.org/, http://icanhazip.com/, https://api.ipify.org/, https://icanhazip.com/`</sub>
...
- **[LLM API pricing list](https://github.com/xyzs996/llm-api-pricing/blob/main/README_JA.md)** — 行に書かれた価格は、リクエスト1回の価格ではありません。他の価格表が落とす列がここにはあります。プロンプトが長くなると切り替わる、2枚目の高い料金表——しかも高いほうがかかるのは、しきい値を超えた分だけでなくそのリクエストの全トークンです。記事で引用したすべての数値も[1 つの表](https://xyzs996.github.io/llm-api-pricing/figures.html)にあり、各行にその出典の文が添えてあります ([JSON](https://cdn.jsdelivr.net/gh/xyzs996/llm-api-pricing@main/data/figures.json) / [CSV](https://cdn.jsdelivr.net/gh/xyzs996/llm-api-pricing@main/data/figures.csv))。まずは[トークン料金が実際どこへ消えるのか](https://xyzs996.github.io/llm-api-pricing/topics/token-optimization.html)から。
